...
def main():
    parser = argparse.ArgumentParser(description="Classify real source_data/overlap_raw images.")
    ...
    # Attach skeleton stats if preprocessing was run.
    stats_csv = config.GENERATED_DIR / "overlap_raw" / "skeleton_stats.csv"
    if stats_csv.exists() and len(df) > 0:
        stats = pd.read_csv(stats_csv)
        stats["file_name"] = stats["file"].apply(lambda x: str(x).split("/")[-1].split("\\")[-1])
